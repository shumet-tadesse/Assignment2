...
import introsde.rest.model.MeasureDefaultRange;
...
public class MeasureDefinition implements Serializable {
	...
	public static MeasureDefinition getMeasureDefinitionByname(String measuretype)
	{
		 ...
		 MeasureDefinition MeasureResult = new  MeasureDefinition();
		 
		 for(int i=0; i< measuredefinitionlist.size();i++)
		 {
			 if(measuredefinitionlist.get(i).getMeasureName().equalsIgnoreCase(measuretype))
				{	
				 MeasureResult= measuredefinitionlist.get(i);
...
//Request #9: GET /measureTypes should return the list of measures your model 
	public static MeasureDefinition getMeasureDefinitionByMeasureType(String mtype) {
		EntityManager em = LifeCoachDao.instance.createEntityManager();
		@SuppressWarnings("unchecked")
		MeasureDefinition mdobj = (MeasureDefinition) em.createQuery(
				"SELECT md FROM MeasureDefinition md where md.measureName='"
						+ mtype + "'").getSingleResult();
		LifeCoachDao.instance.closeConnections(em);
		return mdobj;
	}
